...
export const registerForLiveSession = async (req, res) => {
    const { name, mobile, email, message, razorpay_payment_id, razorpay_order_id, razorpay_signature } = req.body;
    ...
    try {
        const result = await query(
            "INSERT INTO live_session_registrations (name, mobile, email, message, order_id, payment_id) VALUES (?, ?, ?, ?, ?, ?)",
            [name.trim(), String(mobile), email.trim().toLowerCase(), message?.trim() || null, razorpay_order_id, razorpay_payment_id]
        );
